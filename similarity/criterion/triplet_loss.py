...
@registry.CRITERION.register('TripletMarginLoss')
class TripletMarginLoss(nn.Module):

    def __init__(self, cfg):
        super().__init__()
        self.margin = cfg.SIMILARITY.MARGIN
        self.p = cfg.SIMILARITY.P
        self.mining = cfg.SIMILARITY.MINING
    ...
    def forward(self, output_dict, targets):
        ...
        embeddings = output_dict[KEY_OUTPUT]
        triplet_loss, fraction_positive_triplets = self.loss_fn(targets, embeddings, self.margin, self.p)

        return {KEY_LOSS: triplet_loss, "avg_hard_triplets": 100.0 * fraction_positive_triplets}
